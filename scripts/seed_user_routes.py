#!/usr/bin/env python3
"""Idempotent: one public user_created route per users row.
...
def _slug_for_user(user_id: object) -> str:
    return f"user-{str(user_id).replace('-', '')[:12]}"
...
def upsert_user_routes(session: Session) -> int:
    ...
    for index, user in enumerate(users):
        ...
        if existing is not None:
            # Keep author label in sync with display name.
            existing.author_label = user.display_name
            existing.updated_at = _now()
            continue

        template = _ROUTE_TEMPLATES[index % len(_ROUTE_TEMPLATES)]
        slug = _slug_for_user(user.id)
        # Ensure uniqueness within region.
        if session.scalar(select(Route.id).where(Route.region_id == region.id, Route.slug == slug)):
            slug = f"{slug}-{index}"
        ...
        route = Route(
            ...
            estimated_duration_minutes=int(template["estimated_duration_minutes"]),  # type: ignore[arg-type]
            distance_meters=int(template["distance_meters"]),  # type: ignore[arg-type]
