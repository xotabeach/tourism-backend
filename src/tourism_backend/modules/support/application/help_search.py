...
class HelpSearchOut(BaseModel):
    ...
    method: Literal["full_text", "hybrid"] = "full_text"
...
async def search_help(
    ...
    *,
    ...
    # Russian FTS remains available even if semantic retrieval fails.
    ...
    if encoder is not None:
        semantic_ids = await semantic_help_ids(
            session,
            query=query[:400],
        ...
        if semantic_ids is not None:
            ids = fuse_rankings(ids, semantic_ids)
            method = "hybrid"
    ids = ids[:3]
    rows = list(
        (
            await session.scalars(
                select(SupportHelpRevision).where(
                    SupportHelpRevision.id.in_(ids), *visible_help(app_version)
                )
            )
        ).all()
    )
    ...
    return HelpSearchOut(
        available=True, method=method, items=[_out(by_id[id_]) for id_ in ids if id_ in by_id]
    )
    ...
    statement = text(
        "".join(
            (
                ...
                "setweight(to_tsvector('russian', body), 'B'), query) DESC, article_id LIMIT 10",
            )
        )
    )
    return list(
