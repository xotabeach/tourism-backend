...
def get_ai_planning_provider(settings: Settings | None = None) -> AIPlanningProvider:
    ...
    if cfg.ai_provider.value == "deepseek":
        if cfg.deepseek_api_key is None or not cfg.deepseek_api_key.get_secret_value().strip():
            raise RuntimeError("DeepSeek API key is required")
        fallback_models = tuple(
            item.strip() for item in cfg.deepseek_fallback_models.split(",") if item.strip()
        )
