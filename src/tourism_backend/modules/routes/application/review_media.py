"""Validated, metadata-stripping storage for route-review photos."""
...
def _invalid(message: str) -> AppError:
    return AppError(code="invalid_review_image", message=message, status_code=400)
...
async def save_review_image(upload: UploadFile, *, review_id: UUID) -> SavedReviewImage:
    raw = await upload.read(_MAX_BYTES + 1)
    if not raw:
        raise _invalid("Empty upload")
    if len(raw) > _MAX_BYTES:
        raise _invalid("Review image is too large")
    ...
    relative_dir = Path("reviews") / str(review_id)
...
def delete_review_image(storage_key: str, *, review_id: UUID) -> None:
    """Delete only a file inside this review's storage directory."""
    root = _MEDIA_ROOT.resolve()
    allowed = (root / "reviews" / str(review_id)).resolve()
