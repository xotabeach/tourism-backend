"""Validated, metadata-stripping storage for article block images.

Deliberately the same pipeline as ``routes/application/review_media.py``
— same size/pixel caps, same EXIF strip and WebP re-encode, same
"only delete inside this entity's own directory" guard. The one
difference is what an image hangs off: a review owns its photos
directly, while an article's image belongs to a specific
``ArticleBlock``, so files live under ``articles/<article_id>/`` and the
block row holds the attachment reference.
"""
...
def _invalid(message: str) -> AppError:
    return AppError(code="invalid_article_image", message=message, status_code=400)
...
async def save_article_image(upload: UploadFile, *, article_id: UUID) -> SavedArticleImage:
    raw = await upload.read(_MAX_BYTES + 1)
    if not raw:
        raise _invalid("Empty upload")
    if len(raw) > _MAX_BYTES:
        raise _invalid("Article image is too large")
    ...
    relative_dir = Path("articles") / str(article_id)
...
def delete_article_image(storage_key: str, *, article_id: UUID) -> None:
    """Delete only a file inside this article's storage directory."""
    root = _MEDIA_ROOT.resolve()
    allowed = (root / "articles" / str(article_id)).resolve()
