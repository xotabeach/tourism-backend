...
one active place photo already in the DB — rather than leaving it blank.
"""
...
async def covers_for_places(session: AsyncSession, place_ids: list[UUID]) -> dict[UUID, str]:
    ...
    direct = await session.execute(
        select(MediaAttachment.entity_id, MediaAttachment.public_path).where(
    ...
    fallback = await session.execute(
        ...
        .where(
            PlaceImage.place_id.in_(remaining),
...
async def generic_fallback_cover(session: AsyncSession) -> str | None:
    ...
    return await session.scalar(
        ...
        .where(PlaceImage.status == "active", PlaceImage.source_url.is_not(None))
